...
def main():
    ...
    if args.only_test:
        load_dir = (
            os.path.join(args.external_save_dir, "mmrec-reweight-2-2-answer-nonsemantic-2e-4-cosine-4b-instruct-b3-filter8-all-epoch10")
            if args.external_save_dir
            else args.run_name
        )
        resume_from_checkpoint_path_multi_task = os.path.join(load_dir, "weights_epoch_7.pt")
        checkpoint = torch.load(resume_from_checkpoint_path_multi_task, map_location="cpu")
        # ["model_state_dict"]
        model.load_state_dict(checkpoint, False)
        
    load_dir = (
            os.path.join(args.external_save_dir, args.load_run_name)
            if args.external_save_dir
            else args.run_name
        )
    ...
    resume_from_checkpoint_path = os.path.join(load_dir, args.load_weights_name)
    if args.rank == 0:
    ...
    model.lang_encoder.gated_cross_attn_layers.requires_grad_(False)
    model.lang_encoder.get_input_embeddings().requires_grad_(False)
    ...
    if args.subset=="office":
        item_tokens = [f"item_domain_{i}" for i in range(1574)]
        tokenizer.add_tokens(
        item_tokens
        )
        model.lang_encoder.resize_token_embeddings(len(tokenizer))
        args.tokenizer = tokenizer
    elif args.subset=="tool":
        item_tokens = [f"item_domain_{i}" for i in range(6885)]
        tokenizer.add_tokens(
        item_tokens
        )
        model.lang_encoder.resize_token_embeddings(len(tokenizer))
    ...
    total_training_steps = len(multi_instruct_loader) * args.num_epochs // torch.cuda.device_count()
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        ...
        if (epoch+1)%interval==0:
            ...
            if args.rank == 0:
                ...
                # unwrapped_model.save_pretrained(f"{args.external_save_dir}/weights_epoch_{epoch}", state_dict=accelerator.get_state_dict(model))
                accelerator.save(
