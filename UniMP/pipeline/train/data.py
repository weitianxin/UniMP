...
import json
from pipeline.mm_utils.input_dataset import FileDataset
from pipeline.mm_utils.unify_dataset import UnifyDataset
...
def get_mmrec_dataset(args, tokenizer, epoch=0, floor=False, split="train", task="rec"):
    ImageFile.LOAD_TRUNCATED_IMAGES = True
    # dataset = FileDataset(multi_instruct_path, args.selected_cols)
    args.tokenizer = tokenizer
    unified_dataset = RecDataset(args, split=split, task=task)
    # unified_dataset = UnifyDataset(args, dataset)
...
def get_dataset_rec_fn(dataset_type):
    if dataset_type == "mmrec":
        return get_mmrec_dataset
    else:
        raise ValueError(f"Unsupported dataset type: {dataset_type}")
...
def get_data_rec(args, tokenizer, dataset_type, epoch=0, split="train", task="rec"):
    return get_dataset_rec_fn(dataset_type)(args, epoch=epoch, tokenizer=tokenizer, split=split, task=task)