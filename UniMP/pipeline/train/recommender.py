...
def train_one_epoch(
    ...
    for num_steps, (batch_multi_instruct) in tqdm(
        ...
        with accelerator.accumulate(model):
            ...
            #### BACKWARD PASS ####
            loss_multi_instruct = loss_multi_instruct*weights[0]
...
def main():
    ...
    if args.pretrained_model_name_or_path is not None:
        ...
        #     model, args.pretrained_model_name_or_path, device_map="auto"
        # )
        
        # model = OtterForConditionalGeneration.from_pretrained(
        #         "luodian/luodian/otter-9b-hf",
        #         device_map="auto",
    ...
    # item_tokens = [f"item_{j}_{i}" for i in range(50) for j in range(4)]
    # tokenizer.add_tokens(
    #     item_tokens
    # )
    
    # img_tokens = [f"img_{j}_{i}" for i in range(256) for j in range(4)]
    # tokenizer.add_tokens(
    #     img_tokens
    # )
    ...
    multi_instruct_test_loader_img_gen = get_data_rec(args, tokenizer, "mmrec", split="test", task="img_gen")
    ...
    model, optimizer, lr_scheduler, multi_instruct_loader, multi_instruct_eval_loader, multi_instruct_test_loader_rec, multi_instruct_test_loader_exp, multi_instruct_test_loader_img_sel, multi_instruct_test_loader_search, multi_instruct_test_loader_img_gen   = accelerator.prepare(
        ...
        multi_instruct_test_loader_img_sel, multi_instruct_test_loader_search, multi_instruct_test_loader_img_gen
    )
    model.train()
    ...
    if args.single_task:
        if args.task=="rec":
            ...
            multi_instruct_test_loader = multi_instruct_test_loader_rec
        elif args.task=="exp":
            eval_func = eval_model_exp
            multi_instruct_test_loader = multi_instruct_test_loader_exp
        elif args.task=="img_sel":
            eval_func = eval_model_img_sel
            multi_instruct_test_loader = multi_instruct_test_loader_img_sel
        elif args.task=="search":
            eval_func = eval_model_search
            multi_instruct_test_loader = multi_instruct_test_loader_search
        elif args.task=="img_gen":
            eval_func = eval_model_img_gen
            multi_instruct_test_loader = multi_instruct_test_loader_img_gen
        else:
