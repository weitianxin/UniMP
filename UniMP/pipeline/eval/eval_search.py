...
def eval_model_search(
    ...
    for num_steps, (batch_multi_instruct) in tqdm(
        ...
        text_all.append(texts+[output_ids])
        ...
        # if args.rank==0 and num_steps<10:
        #     print(texts, output_ids)
        r = np.array([0]*10)
    ...
    hr_all, ndcg_all, mrr_all = accelerator.gather((torch.tensor(hr).to(device_id), torch.tensor(ndcg).to(device_id), torch.tensor(mrr).to(device_id)))
